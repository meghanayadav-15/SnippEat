  ...
  if (managingCats) return (
    <div style={{ fontFamily:'sans-serif', maxWidth:660, margin:'0 auto', padding:'20px 20px 60px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <h2 style={{ fontSize:20, fontWeight:900, margin:0 }}>🏷 My Categories</h2>
      ...
      <div style={{ background:'#fff', borderRadius:20, border:'2px solid #ede4d4', padding:'20px', marginBottom:16 }}>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:10 }}>
          ...
          <input value={newCat.name} onChange={e=>setNewCat(p=>({...p,name:e.target.value}))}
            ...
            style={{ flex:1, padding:'10px 12px', borderRadius:10, border:'2px solid #ede4d4', fontSize:14, outline:'none' }}/>
        </div>
        <div style={{ display:'flex', gap:8, alignItems:'center', marginBottom:14 }}>
          ...
          <input type="text" value={newCat.color} onChange={e=>setNewCat(p=>({...p,color:e.target.value}))}
            placeholder="#hex"
            style={{ width:72, padding:'6px 8px', borderRadius:8, border:'2px solid #ede4d4', fontSize:12, outline:'none' }}/>
        </div>
        <button onClick={addCategory}
          style={{ width:'100%', padding:'12px', borderRadius:12, background:'#e8401c', border:'none', color:'#fff', fontSize:14, fontWeight:800, cursor:'pointer' }}>
          + Add Category
        </button>
      </div>
    ...
    <div style={{ fontFamily:'sans-serif', background:'#fffbf5', minHeight:'100vh' }}>
      ...
      <div style={{ maxWidth:920, margin:'0 auto', padding:'24px 20px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <h2 style={{ fontSize:20, fontWeight:900, margin:0 }}>My Library 📚</h2>
          <button onClick={()=>setManagingCats(true)} style={{ padding:'7px 14px', borderRadius:10, background:'#fff8ee', border:'2px solid #ede4d4', cursor:'pointer', fontSize:12, fontWeight:800, color:'#6b4f2a' }}>🏷 Categories</button>
        </div>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:8, flexWrap:'wrap' }}>
          <span style={{ fontSize:11, color:'#a0896a', fontWeight:800, alignSelf:'center', textTransform:'uppercase' }}>Source:</span>
          {[['All','All'],['website','🌐 Web'],['youtube','▶️ YouTube'],['text','📋 Manual']].map(([val,label])=>(
            <button key={val} onClick={()=>setSourceFilter(val)} style={{ padding:'7px 14px', borderRadius:10, border:'2px solid #ede4d4', cursor:'pointer', fontSize:12, fontWeight:800, background:sourceFilter===val?'#e8401c':'#fff', color:sourceFilter===val?'#fff':'#a0896a' }}>{label}</button>
          ))}
        </div>

        <div style={{ display:'flex', gap:8, marginBottom:20, flexWrap:'wrap' }}>
          <span style={{ fontSize:11, color:'#a0896a', fontWeight:800, alignSelf:'center', textTransform:'uppercase' }}>Filter:</span>
          <button onClick={()=>setCatFilter('All')} style={{ padding:'7px 14px', borderRadius:10, border:'2px solid #ede4d4', cursor:'pointer', fontSize:12, fontWeight:800, background:catFilter==='All'?'#e8401c':'#fff', color:catFilter==='All'?'#fff':'#a0896a' }}>All</button>
          <button onClick={()=>setCatFilter('favourites')} style={{ padding:'7px 14px', borderRadius:10, border:'2px solid #ede4d4', cursor:'pointer', fontSize:12, fontWeight:800, background:catFilter==='favourites'?'#fbbf24':'#fff', color:catFilter==='favourites'?'#fff':'#a0896a' }}>⭐ Favourites</button>
          {categories.map(c=>(
            <button key={c.id} onClick={()=>setCatFilter(c.id)} style={{ padding:'7px 14px', borderRadius:10, border:`2px solid ${catFilter===c.id ? c.color : '#ede4d4'}`, cursor:'pointer', fontSize:12, fontWeight:800, background:catFilter===c.id?c.color:'#fff', color:catFilter===c.id?'#fff':'#6b4f2a' }}>
              {c.emoji} {c.name}
            </button>
          ))}
        </div>
              ...
              return (
                <div key={r.id} onClick={()=>setSelected(nr)} style={{ background:'#fff', borderRadius:20, border:'2px solid #ede4d4', overflow:'hidden', cursor:'pointer', transition:'all 0.2s', position:'relative' }}
                  ...
                  <div style={{ padding:'12px 14px' }}>
                    <h3 style={{ fontSize:14, fontWeight:800, margin:'0 0 4px', lineHeight:1.3 }}>{nr.title}</h3>
                    {(nr.categoryIds||[]).length > 0 && (
                      <div style={{ display:'flex', gap:4, flexWrap:'wrap', marginTop:4 }}>
                        {nr.categoryIds.map(cid=>{
                          const cat = categories.find(c=>c.id===cid);
                          if (!cat) return null;
                          return <span key={cid} style={{ fontSize:10, fontWeight:800, padding:'2px 7px', borderRadius:99, background:cat.color, color:'#fff' }}>{cat.emoji} {cat.name}</span>;
                        })}
                      </div>
                    )}
                  </div>
